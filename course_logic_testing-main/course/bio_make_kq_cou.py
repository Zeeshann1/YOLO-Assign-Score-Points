...
class BIO_make_kq(ConfigModel):
    def __init__(self):
        super(BIO_make_kq, self).__init__()
        self.initScore()
        ...
        if top_true and front_true and side_true:
            ...
            if ret:
                if score > self.max_score_si:
                    ...
                    for view_ in ["front", "side"]:
                        if view_ != view:
                            assign_score_fun(4, view_)
                            break
                    return
            # 4. 刮
            ret, view, score = self.j4()
            if ret:
                if score > self.max_score_gua:
                    self.max_score_gua = score
                    self.exp_ok[2] = self.frame_id
                    self.exp_ok[3] = self.frame_id
                    assign_score_fun(3, view)
                    for view_ in ["front", "side"]:
            ...
            if (self.frame_id > 1500 and self.has_score()):
                ret, iou_ = self.j10()
                if ret:
                    if iou_ <= self.iou_ and not self.scorePoint10:  # todo
                        self.iou_ = iou_
    ...
    def j1(self):
        # 擦拭载玻片和盖玻片
        j1_judge = (self.scorePoint6 or self.scorePoint7 or self.scorePoint8 or self.scorePoint9)  # todo
        if j1_judge:
            return False, None, 0.
        def j_top():
    ...
    # DONE
    def j3(self):
        if "喝水" in self.d["front"].keys():
            return True, "front", self.d["front"]["喝水"][0][4]
        if "喝水" in self.d["top"].keys():
            return True, "top", self.d["top"]["喝水"][0][4]
        if "喝水" in self.d["side"].keys():
            return True, "side", self.d["side"]["喝水"][0][4]
        if "吐水" in self.d["front"].keys():
            return True, "front", self.d["front"]["吐水"][0][4]
        if "吐水" in self.d["top"].keys():
            return True, "top", self.d["top"]["吐水"][0][4]
        if "吐水" in self.d["side"].keys():
            return True, "side", self.d["side"]["吐水"][0][4]
        return False, None, 0.

    def j4(self):
        # 用镊子从洋葱鳞片叶的内侧撕取一小块透明薄膜
        if "取样" in self.d["front"].keys():
            return True, "front", self.d["front"]["取样"][0][4]
        if "取样" in self.d["top"].keys():
            return True, "top", self.d["top"]["取样"][0][4]
        if "取样" in self.d["side"].keys():
            return True, "side", self.d["side"]["取样"][0][4]
        return False, None, 0.
